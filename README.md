...
Quando a origem e o `Model` possuem os mesmos nomes de atributos, tipos compatíveis e a mesma estrutura, pode ser utilizado um mapper genérico `Mapper<Source, Model>` para reduzir código repetitivo. Ele guarda o par de tipos da fronteira, delega a cópia dos atributos a um `ObjectMapper` interno e resolve `Source` e `Model` pelo ponto de injeção. O exemplo completo de implementação — a classe, a fábrica `prototype` e o uso no adaptador de entrada — está em [Generic Mapper.md](Generic%20Mapper.md).

Todo mapper — o `Mapper<Source, Model>` injetado direto ou um mapper nomeado — parte da mesma base: `Mapper<Source, Model>`. Não existem dois conceitos separados (mapper genérico vs. mapper escrito do zero); existe um espectro de sobrescrita sobre a mesma classe:

- **Sem sobrescrita** — a fronteira injeta `Mapper<Source, Model>` tipado direto (sem subclasse) e chama `toModel`/`fromModel` ou `toModels`/`fromModels` para coleções (com `Consumer` opcional para ajuste simples por item). É a preferência padrão: só se cria uma subclasse quando pelo menos uma direção da fronteira precisa de tratamento especial.
- **Sobrescrita parcial** — a fronteira precisa de tratamento especial em algum campo (nome diferente, transformação pontual, `null` condicional), mas o resto é automático. Um mapper nomeado (`UserMapper`, `UserEntityMapper`) estende `Mapper<Source, Model>`, chama `super(Source.class, Model.class)` no construtor e sobrescreve `toModel`/`fromModel`, podendo chamar `super.toModel(source)`/`super.fromModel(model)` para a parte automática e ajustar só os campos com regra.
- **Sobrescrita total** — a conversão automática não é possível (estrutura muito diferente) ou é mecanicamente possível mas esconde acoplamento indesejado (ex.: um campo cuja conversão automática dependeria de uma anotação pensada para outra fronteira). O mapper nomeado estende `Mapper<Source, Model>` e sobrescreve `toModel`/`fromModel` por completo, sem chamar `super`.

A classificação vale **por direção**, não por classe: um mesmo mapper nomeado pode ser "sem sobrescrita" em uma direção (deixada herdada da base) e "sobrescrita total" na outra (sobrescrita por completo). Dentro de uma direção sobrescrita, o que separa "parcial" de "total" é a chamada a `super`: se o override chama `super.toModel`/`super.fromModel` para parte do trabalho, é sobrescrita parcial; se não chama `super` em nenhum ponto, é sobrescrita total. Ao descrever um mapper, diga qual direção recebeu qual tratamento (ex.: "sobrescrita total em `toModel`, `fromModel` herdado") em vez de rotular a classe inteira.

Em qualquer sobrescrita, o mapper nomeado usa exatamente os nomes `toModel`/`fromModel` — nunca um nome próprio (como `paraModel`, `converter` etc.). Isso não é só estilo: `toModels`/`fromModels` (com `Consumer`) são herdados da base e chamam `this.toModel`/`this.fromModel` internamente; um método com outro nome não é override, e a coleção continuaria usando a conversão automática da base em vez da lógica da subclasse — um bug silencioso, mais grave ainda no caso de sobrescrita total.

O único caso em que a cópia automática não funciona mecanicamente é nome de atributo diferente entre `Source` e `Model` — o Jackson copia por nome. Nos demais casos (tipo diferente, objeto aninhado, coleção), a cópia automática costuma funcionar, mas cabe avaliar se ela esconde acoplamento antes de deixar sem sobrescrita: se dois modelos só convertem automaticamente porque um deles usa uma anotação pensada para outra fronteira (ex.: contrato HTTP), a conversão automática acopla as duas fronteiras por acidente — nesse caso, prefira a sobrescrita total mesmo que a automática "funcionasse".

O `Mapper<Source, Model>` (e suas subclasses) só é utilizado nas fronteiras autorizadas (`presentation.mapper`, `data.mapper`, adaptadores de entrada e implementação do `Datastore`) e nunca no `Core` ou no `Service`, pois sua implementação depende de reflection ou bibliotecas de mapeamento. O bean genérico sem subclasse é `prototype`: cada ponto de injeção carrega um par de tipos distinto e recebe a própria instância; os genéricos precisam ser concretos no campo ou parâmetro. Uma subclasse nomeada não passa pela fábrica `prototype` — ela mesma fornece os tipos ao construtor da base.

A classe base `Mapper<Source, Model>` e sua fábrica são infraestrutura compartilhada e ficam em um pacote neutro, fora de `presentation` e de `data` (ex.: `<base>.mapper` ou `shared.mapper`). As duas fronteiras estendem a mesma base: se ela morasse em `presentation.mapper`, um mapper nomeado de `data.mapper` passaria a importar `presentation` só para herdar da base, criando uma dependência da Data para a Presentation que a arquitetura proíbe. Nesse pacote neutro ficam apenas a base e a fábrica; os mappers nomeados continuam cada um na sua fronteira (`presentation.mapper`, `data.mapper`).
